...
void setup() {
  ...
  model_feature_buffer = interpreter->input(1)->data.uint8;
...
void loop() {
  ...
  if (useSampleData || receivedNewSample) {
    ...
    if (receivedNewSample) {
      ...
      float sc2 = interpreter->input(1)->params.scale;
      int32_t z_point2 = interpreter->input(1)->params.zero_point;
      ...
      if (CALCULATE_FEATS) {
        ...
        model_feature_buffer[0] = feat_mean[0] / sc2 + z_point2;
        model_feature_buffer[1] = feat_mean[1] / sc2 + z_point2;
        model_feature_buffer[2] = feat_mean[2] / sc2 + z_point2;
        post_mean = millis();
        ...
        MicroPrintf("Quant means: %d, 2: %d, 3: %d\n",model_feature_buffer[0],model_feature_buffer[1],model_feature_buffer[2]);
        ...
        model_feature_buffer[3] = feat_std[0] / sc2 + z_point2;
        model_feature_buffer[4] = feat_std[1] / sc2 + z_point2;
        model_feature_buffer[5] = feat_std[2] / sc2 + z_point2;

        MicroPrintf("Stdevs = %f, %f, %f",feat_std[0],feat_std[1],feat_std[2]);
        MicroPrintf("Quant_std 1 = %d, %d, %d",model_feature_buffer[3],model_feature_buffer[4],model_feature_buffer[5]);
        post_std = millis();
        ...
        model_feature_buffer[6] = feat_mae[0] / sc2 + z_point2;
        model_feature_buffer[7] = feat_mae[1] / sc2 + z_point2;
        model_feature_buffer[8] = feat_mae[2] / sc2 + z_point2;

        MicroPrintf("mae = %f, %f, %f",feat_mae[0],feat_mae[1],feat_mae[2]);
        MicroPrintf("Quant_mae 1 = %d, %d, %d",model_feature_buffer[6],model_feature_buffer[7],model_feature_buffer[8]);
        post_mae = millis();
        ...
        model_feature_buffer[9] = feat_rms / sc2 + z_point2;

        MicroPrintf("rms = %f",feat_rms);
        MicroPrintf("Quant_rms = %d",model_feature_buffer[9]);
        post_rms = millis();
        ...
        for (int i=0;i<10;i++){
          model_feature_buffer[10+i] = feat_hist[0][i] / sc2 + z_point2;
          model_feature_buffer[20+i] = feat_hist[1][i] / sc2 + z_point2;
          model_feature_buffer[30+i] = feat_hist[2][i] / sc2 + z_point2;
        }
        ...
        MicroPrintf("Quant_hist = %d, %d, %d, %d, %d, %d, %d, %d, %d, %d",model_feature_buffer[10],model_feature_buffer[11],model_feature_buffer[12],model_feature_buffer[13]
                    ,model_feature_buffer[14],model_feature_buffer[15],model_feature_buffer[16],model_feature_buffer[17],model_feature_buffer[18],model_feature_buffer[19]);
        post_hist = millis();
        ...
        for (int i=0;i<kFeatureElementCount;i++) {
          model_feature_buffer[i] =  g_input_buffer[i+kInputElementCount] / sc2 + z_point2;
        }
      }
      expected_result = std::round(g_input_buffer[kInputElementCount+kFeatureElementCount]);
      // MicroPrintf("exp: %d", (int)(g_input_buffer[kInputElementCount+kFeatureElementCount]));
    ...
    // With test data
     ...
     for (int i=0;i<kInputElementCount;i++) {
      ...
      for (int i=0;i<kFeatureElementCount;i++) {
        model_feature_buffer[i] = test_input2[i];
      }  
      expected_result = 4;   
    }    
