...
class TestOverSerial:
 ...
 # Define constants
  ...
  g_len_in2 = 40
  g_len_label = 1
  g_len_sample = g_len_in1+g_len_in2+g_len_label
  g_num_samples = 2993
  #
  # private types
  #
  ...
  def _send_data(self, data_type: "TestOverSerial.DataType", datum: bytes,
    ...
    main = Main()
    main.log_console("Preparing to send data")
  ...
  def _get_csv_rows_count(self,file_path):
      with open(file_path, 'r') as csvfile:
          csv_reader = csv.reader(csvfile)
          return sum(1 for row in csv_reader)

  def _load_csv(self, path: Path) -> bytes:
    num_samples = self._get_csv_rows_count(path)
    len_sample = self.g_len_in1+self.g_len_in2+self.g_len_label
    samples = np.zeros(shape=(num_samples,len_sample),dtype='float32')
    try:
      with open(path, 'r') as csvfile:
        ...
        for i,row in enumerate(csv_reader):
            for j,element in enumerate(row):
              samples[i,j] = element
        return [samples.tobytes(),samples[:,0],samples[:,-1]]
  ...
  def start_tests(self) -> None:
    ...
    try:
      ...
      test_info = list(config[TestOverSerial.ConfigKeys.DATA])[0]
      # self._results.total = len(test_list)
      self._results.total = self.g_num_samples
      test_index = 1
      ...
      if  data_type == TestOverSerial.DataType.RAW_FLOAT:
        [datum,firstval_list,label_list] = self._load_csv(file_path)
      else:
        main.fatal(f"Data type <{data_type}> not yet supported")
      for i in range(self.g_num_samples-1):
        lb_datum = i*self.g_len_sample*4
        ub_datum = (i+1)*self.g_len_sample*4
        current_datum = datum[lb_datum:ub_datum]
        label = int(label_list[i])
        firstval = firstval_list[i]

        main.log_console(
            f"Test #{test_index} for label <{label}> file <{file_path.name}>")
        # main.log_console(f"First value: {firstval}")
        ...
        self._results.total_time += int(groups[2])
...
class SerialWrapper:
  ...
  def initialize(self, port: Union[str, None]) -> None:
    ...
    try:
      self._serial = serial.Serial(port=port, baudrate=76800,exclusive=True)
...
class Main:
  ...
  def _parse_arguments(self) -> argparse.ArgumentParser:
    ...
    parser.add_argument("--example",
                        ...
                        choices=[
                            ...
                            str(TestOverSerial.Examples.HAR)
                        ],
  ...
  def _load_config(self):
    if self._args.config is None:
      if self._args.example == TestOverSerial.Examples.HAR:
        self._args.config = Path(
        f"C:/Users/samue/Documents/Arduino/Sketches/{self._args.example}/data/serial_test_config.json")
      else:
        self._args.config = Path(
  ...
  def main(self) -> bool:
    ...
    try:
      ...
      accuracy = (results.passed/results.completed)*100
      ...
      self.log_console(
          ...
          f"passed={results.passed} failed={results.failed} accuracy={accuracy} | "
          f"average_time={avg_time}ms")
